 ...
 * @param {string} inner the already-joined argument text
 * @returns {{ raw: number[], alpha: number } | null} the channels, or null
 */
...
const _minifyPolarColorFunction = (
	...
	return _colorFromChannels(raw, alpha, margin, hexAlpha, forFallback);
};
 ...
 * @param {number} alpha the alpha, 0..1
 * @param {number} margin how far from a `.5` boundary a channel must sit
 ...
 */
const _colorFromChannels = (raw, alpha, margin, hexAlpha, forFallback) => {
	/** @type {number[]} */
	const channels = [];
	let clipped = false;
	for (const value of raw) {
		const byte = forFallback ? _clamp255(value) : _channelByte(value, margin);
 ...
 * @returns {{ linear: number[], alpha: number } | null} the color, or null
 */
const _resolveColor = (text) => {
	...
	if (text.charCodeAt(0) === CC_NUMBER_SIGN) {
		...
		if (lower === "transparent") return { linear: [0, 0, 0], alpha: 0 };
...
const _foldColorMix = (inner, hexAlpha, forFallback) => {
	...
	/** @type {number[]} */
	const weights = [];
	/** @type {{ linear: number[], alpha: number }[]} */
	const colors = [];
	for (let i = 1; i <= 2; i++) {
	...
	return _colorFromChannels(
		raw,
		alpha,
		space.direct ? _ROUNDING_MARGIN : _LAB_ROUNDING_MARGIN,
		hexAlpha,
		forFallback
	);
};
...
const _foldRelativeColor = (fn, inner, hexAlpha, forFallback) => {
	...
	let write;
	if (fn === "color") {
		const space = COLOR_SPACE_MODEL.get(asciiLowerCaseName(parts[at++] || ""));
		...
		const space =
			/** @type {{ to: (c: number[]) => number[], from: (c: number[]) => number[], hue: number }} */ (
				COLOR_SPACE_MODEL.get(fn)
			);
		channels = space.to(origin.linear).map((c, i) => c * shape.scale[i]);
		write = (/** @type {number[]} */ c) =>
			space.from(c.map((value, i) => value / shape.scale[i]));
	}
	/** @type {Map<string, number>} */
	const bound = new Map();
