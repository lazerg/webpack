...
/**
 * Whether this function computes with the colors written in it rather than
 * painting one of them: a mix of two, or a relative color reading its origin's
 * channels. Inside one, a color rounded to the byte an engine paints is the
 * wrong color — `light-dark()` picks one of its two and paints it, so it is not
 * one of these.
 * @param {number} i0 the function node's index
 * @returns {boolean} true inside such a call
 */
const _computesColors = (i0) => {
	const name = toLowerCaseIfNeeded(_input.slice(_starts[i0], _aux0[i0]));
	if (COLOR_COMPUTING_FUNCTIONS.has(name)) return true;
	// A relative color is spotted by the `from` its arguments open with, whichever
	// color function states it.
	return (
		_RELATIVE_CHANNELS.has(name) &&
		// Past the `(` the name ends at, which is where the arguments start.
		_RELATIVE_FROM_RE.test(_input.slice(_aux0[i0] + 1, _ends[i0]))
	);
};
...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		const previousComputed = _inComputedColor;
		if (
			ty === T_FUNCTION &&
		...
		if (ty === T_FUNCTION && !_inComputedColor && _computesColors(i0)) {
			_inComputedColor = true;
		}
		for (let i = vs; i < ve; i++) {
			_walkValue(_nodeRef(_flat[i]), node, i - vs, writer);
		}
		_inGradient = previousGradient;
		_inComputedColor = previousComputed;
	}
...
const _emitTopLevel = (writer, node, offset, line, column, text, own) => {
	...
	const opener =
		printing && _transforms.mergeRules && _types[_nodeIndex(node)] === T_AT_RULE
...
const grammar = (input, visitors, writer, options) => {
	...
	_colorFallbacksNeeded =
		!_hwbColorsAllowed || !_labColorsAllowed || !_oklabColorsAllowed;
	...
	try {
		...
		if (_blockContentsNodes === null) {
			...
			// declaration wins.
			if (_lightDarkUsed && writer !== undefined) {
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties" | "renderEmbeddedSource" | "deferEmbeddedSource" | "transforms" | "unusedSymbols" | "pseudoClasses">} CssPrintOptions
 */
 ...
 * three decimals or fewer, so the search always ends.
 * @param {number} a alpha 0..255
...
const _hexAlphaFallback = (text) => {
	...
	return `rgba(${parts[0]},${parts[1]},${parts[2]},${_alphaByteText(parts[3])})`;
};
...
const _minifyColorFunction = (fn, inner, hexAlpha) => {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (i === 3) {
			...
			written.push(pct ? (v * 255) / 100 : v);
			channels.push(_clamp255(written[written.length - 1]));
		}
	}
	...
	// Left where a mix or a relative reference reads them: `rgb()` is the one
	// color form that reaches its channels without a conversion, so it records
	// them here rather than through `_colorFromChannels`.
	_fallbackChannels = written;
	_fallbackAlpha = alpha;
	_fallbackClipped = false;
	_fallbackConversion = ENCODED_ALREADY;
	// Inside a color an engine computes with rather than paints, the byte is the
	// wrong color: the engine mixes, interpolates or reads channels from what was
	// written, so rounding one moves the answer instead of the last bit of a
	// pixel.
	if (_inComputedColor && written.some((value) => value !== _clamp255(value))) {
		return null;
	}
...
const _sameVocabulary = (later, earlier) => {
	...
	// ...or a color named where the earlier one names or spells one too. Only a
	// name as old as naming a color: the few that arrived later are ones an
	// engine may read none of while reading the rest.
...
// Chromium's sits up to 1.03e-4 above it, in encoded units.
const _TRANSFER_ERROR = 1.65e-4;

// ...and one that runs a matrix as well carries that approximation into every
// term of the product, so what it costs is relative to the color's own
// magnitude: up to 9.46e-4 of it, across every space and the Lab family.
//
...
// only the engines it can be run against have been measured. Re-run it to derive
// them again, for a newer Chromium or for an engine it has not seen.
const _CONVERSION_ERROR = 1.51e-3;
...
const _predefinedColorChannels = (inner) => {
	...
	let alpha = 1;
	if (args.length === 5) {
		const parsed = _COLOR_NUMBER_RE.exec(args[4]);
		if (parsed === null) return null;
...
const _wideGamutColor = (raw, alpha) => {
	...
	const opacity = alpha < 1 ? `/${_alphaByteText(_clamp255(alpha * 255))}` : "";
...
const _colorFromChannels = (raw, alpha, conversion, hexAlpha, forFallback) => {
	...
	const channels = [];
	let clipped = false;
	// A color an engine computes with rather than paints stands only as a
	// spelling naming it exactly: a conversion of any kind, or a channel that has
	// to round, moves the answer rather than one pixel's last bit.
	if (
		!forFallback &&
		_inComputedColor &&
		(conversion !== ENCODED_ALREADY ||
			raw.some((value) => value !== _clamp255(value)))
	) {
		return null;
	}
	...
	if (alpha < 1) {
		...
		const opacity = _clamp255(alpha * 255);
		return hexAlpha
		...
		const written =
			fn === "color"
				...
				: fn === "rgb" || fn === "rgba"
					...
					: fn.replace(/a$/, "");
		if (
			_minifyColorFunction(fn, inner, true) === null &&
...
const _hueEndpoints = (from, to) => {
	...
	// Both are already wrapped to 0..360, so the shorter arc is the one under a
	// half turn — which is what every method is stated as a correction to.
...
const _foldColorMix = (inner, hexAlpha, forFallback) => {
	...
	if (space === undefined) return null;
	// `in <space> <method> hue` — the two words after the space, or nothing.
	_hueMethod = "";
	if (head.length === 4) {
		if (!equalsLowerCase(head[3], "hue")) return null;
		_hueMethod = asciiLowerCaseName(head[2]);
	...
	else if (weights[0] < 0) weights[0] = Math.max(0, 100 - weights[1]);
	else if (weights[1] < 0) weights[1] = Math.max(0, 100 - weights[0]);
	...
	// No byte, either because the color is outside sRGB or because its rounding
	// is one an engine could go the other way on. Where both colors were written
	// in the space they were mixed in, the mix itself converted nothing, so it
	// can be written there instead of left as the call.
...
const _relativeChannel = (text, bound, angle) => {
	...
	// an expression over one reduces to no unit at all — which a hue slot reads as
	// degrees. An angle stands there too, and mixing the two is no value: that
	// leaves a sum of two units, which is refused above.
	const unit = only[0][0];
...
// The pair a lowered `light-dark()` reads, and the rule that defaults them.
// `initial` is the guaranteed-invalid value, so a `var()` naming a property set
// to it takes its fallback; one set to nothing substitutes nothing. Between them
// the two say which half of the function is written out.
...
// `*` included — overrides it wherever it stands. That is what lets it be
// written last rather than first.
...
const _FALLBACK_COLOR_FUNCTION_RE =
	/(^|[^-\w])(hwb|lab|lch|oklab|oklch|color)\(([^()]*)\)/gi;
...
const _colorFallbackDeclaration = (text, colon) => {
	...
	// reading neither, this for one reading no Lab family, and the author's own
	// for the rest.
...
const _wideGamutFallback = (value) => {
	...
	const written = value.replace(
		...
		(whole, before, name, inner) => {
			const fn = asciiLowerCaseName(name);
			if (!_unreadableColorFunction(fn)) return whole;
			const read = _minifyPolarColorFunction(fn, inner, _hexAlphaAllowed, true);
...
const _inFontFamily = () =>
	_valueDeclaration !== null &&
	FAMILY_LIST_PROPERTIES.has(asciiLowerCaseName(A.name(_valueDeclaration)));
 ...
 * printed parts, in place — so a script can apply the class where the engine
 * reads no such pseudo. Only a plain one: a functional pseudo prints with its
 * arguments attached and a pseudo-element carries a second colon, and neither is
 * what a class stands in for.
 * @param {string[]} parts the prelude's printed parts
 * @returns {void}
 */
const _replacePseudoClasses = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (parts[i].includes("(")) {
			...
			// selectors of their own — `:not(:hover)` is the same question one level
			// down. Read as text, so a quoted attribute value (where a `:name` is
			// nobody's pseudo) leaves the whole argument alone.
			...
			const written = named.get(toLowerCaseIfNeeded(parts[i]));
...
// One printed `(<feature><comparison><value>)` or
// `(<value><comparison><feature><comparison><value>)`, the two shapes the range
// spelling prints. A bound holds no comparison of its own, so the greedy runs
// stop at the next one and a `calc()` bound still reads as one bound.
...
const _minMaxCondition = (feature, comparison, value) =>
	`(${comparison === "=" ? "" : comparison === ">=" ? "min-" : "max-"}${feature}:${value})`;
...
const _shortestFlexValue = (components) => {
	...
	// itself: §7.1.1 reads a unitless zero not preceded by two factors as a
	// factor, so `1 1 0` is not `1 0`.
...
const _positionPercentage = (written, axis) => {
	...
	// A zero is the same place however it is spelled, and the only length that
	// resolves without knowing the box.
	if (_ZERO_COMPONENT_RE.test(lower) || lower === "0%") return "0%";
...
const _dropLayerInitials = (property, components) => {
	...
	if (
		...
		let sized = true;
		if (slash !== -1) {
			// The size is the one or two components after the `/` that are one, and
			// it goes only where every one of them is the size an unwritten slot
			// takes — the next slot's own keywords stand right after them, and none
			// of those is a `<bg-size>`.
			let last = slash;
...
const _collapseShorthand = (path, property, node, writer) => {
	...
	// A `text-decoration` slot the target does not read is one the engine drops
	// the whole declaration for, so taking that slot out would bring the rest of
	// it back to life where the author wrote nothing.
	const readable =
	...
	// list of spellings, so it answers for a color and a length where the keyword
	// table cannot; every other shorthand keeps that table.
	// A layered shorthand's position, size and boxes are read positionally, and
	// the keyword table then answers for what is left of the layer.
	...
	if (property === "display" && components.length === 2) {
		...
		if (
			short !== undefined &&
			(short.length < pair.length || !_displayTwoValuesAllowed)
		) {
			return [short];
		}
	}
...
const _mergeBoxLonghands = (
	...
	for (const [shorthand, longhands] of table) {
		...
		// which loses the slots that were fine. Not asked of an ordered grammar,
		// whose slots take different types by position (`flex:1 1 0px`).
		...
		if (mode === MERGE_FAMILY) {
			...
			value = (shortest === null ? values : shortest).join(" ");
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	if (preludesEqual) {
		...
		// One block holds a property once, so a property both of them declare joins
		// only where the later declaration is one the earlier could not have been
		// read behind anyway — the same question a repeated property inside one
		// block answers. A shorthand holds every longhand its name prefixes and
		// `all` holds the lot, and what those two say to each other is not this.
		const earlier = _blockDeclarations(body);
		if (earlier === null) return null;
		// The later block's own declarations, where it holds nothing else: a rule
		// of its own leaves the properties readable but not the values, and
		// without those a repeated property is one this cannot answer for.
		const later = _transforms.removeDeadRules ? _blockDeclarations(rest) : null;
		const names =
			later === null
				...
				if (one.property === other) {
					const value =
						later === null
							? null
							: /** @type {{ value: string }} */ (
									later.find((held) => held.property === other)
								).value;
...
const _isNamedLayerAtRule = (node) => {
	...
	return (
		_types[i] === T_AT_RULE &&
		rangeEqualsLowerCase(_input, _starts[i] + 1, _aux0[i], "layer")
	);
};
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (minify && path.type() === T_AT_RULE) {
		...
		const container = equalsLowerCase(name, "container");
		// Every prelude that is a chain of conditions, whichever rule states it.
		if (
			_transforms.shortenMediaQueries &&
		...
		if (media || container) {
			...
			if (media) {
				...
				if (_transforms.shortenMediaQueries) _dropImpliedMediaType(parts);
			}
			...
			else if (media && _transforms.lowerUnsupported) _useMinMaxSpelling(parts);
		}
	}
	...
	if (
		...
		minify &&
		path.type() === T_AT_RULE &&
		KEYFRAMES_AT_RULE_RE.test(path.name()) &&
...
const _expandTextDecoration = (value, important) => {
	...
	let out = `text-decoration:${line === undefined ? "none" : line}${important};`;
	for (const longhand of longhands) {
		if (longhand === "text-decoration-line") continue;
		const written = filled.get(longhand);
		if (written !== undefined) out += `${longhand}:${written}${important};`;
	}
	return out;
};
...
const _expandUnsupportedShorthand = (text) => {
	...
	for (let i = 0; i < longhands.length; i++) {
		out += `${longhands[i]}:${values[i]}${important};`;
	}
	return out;
};
...
const _composeBlockBody = (
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (earlier === undefined) {
				...
				for (let j = earlier.length - 1; j >= 0; j--) {
					...
					if (
						names
							? !_coveredByLater(value, written)
							: !_sameVocabulary(value, written)
					) {
						continue;
					}
	...
	if (minify && _colorFallbacksNeeded && _transforms.colorFallbacks) {
		/** @type {Set<string>} */
		...
		for (let i = 0; i < items.length; i++) {
			...
			const colon = text.indexOf(":");
			const property = _printedProperty(text, colon);
			// An earlier declaration of the same property already is the fallback the
			// author wrote, whatever it says.
			if (!alreadyWritten.has(property) && text.includes("(")) {
	...
	for (let i = 0; i < texts.length; i++) {
		...
		if (minify && !isBlock) {
			const expanded = _transforms.lowerUnsupported
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_DIMENSION: {
			...
			// A declaration value, or a media feature — which is a `<length>`, a
			// `<ratio>` or an `<integer>` and nothing else. Everywhere else a prelude
			// number is An+B (`2n+1`) or similar, where stripping a `+` breaks the
			// selector; a `style()` query's own tokens never reach here, its call
			// printing them as written.
			return minify && (path.inValue() || _inMediaConditionPrelude)
		...
		case T_FUNCTION: {
			...
			if (
				...
				// A half the target cannot read would be substituted into a `var()`
				// and make the whole declaration invalid at computed-value time — and
				// nothing standing before it is read then, so this is left alone
				// rather than lowered into something no fallback can answer for.
			...
			if (!_inSupportsPrelude && !_inCustomProperty) {
				...
				if (term !== null) {
					...
					// which is longer than the expression that produced it. Only the
					// outermost call is asked: a nested one is a term the expression
					// around it still has to reduce, and that is what is measured.
					if (
						_mathFunctionDepth > 1 ||
			...
			if (path.inValue() && !_inSubstitutedValue) {
				...
				if (!_doublePositionAllowed) {
					const split = !_transforms.lowerUnsupported
						? null
		...
		case T_QUALIFIED_RULE: {
			...
			const unusedRule = _takeUnusedRule(/** @type {Node} */ (_currentNode));
			...
			if (
				minify &&
				// An empty rule paints nothing, so dropping it leaves the cascade as
				// it was — but only where the block itself carries no meaning (see
				// `DROPPABLE_WHEN_EMPTY_AT_RULES`). `_streamClose` asks the same.
				_transforms.removeDeadRules &&
				body.length === 0 &&
			...
			const text = `${prelude}${soft}{${scheme}${nl}}${schemeCopy}`;
		...
		case T_HASH: {
			...
			if (_hexAlphaAllowed) return text;
			const lowered = _transforms.lowerUnsupported
...
let _displayTwoValuesAllowed = true;
let _systemUiAllowed = true;
...
let _loweringLightDark = false;
let _lightDarkUsed = false;
let _sourceNamesLightDark = false;
