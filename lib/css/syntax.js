...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (minify && path.type() === T_AT_RULE) {
		...
		if (media || container || equalsLowerCase(name, "supports")) {
		...
		if (media || container) {
			...
			else if (media) _useMinMaxSpelling(parts);
		}
	}
	if (minify && path.type() === T_QUALIFIED_RULE) {
		...
		} else if (!_notSelectorListAllowed || !_langArgumentListAllowed) {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			if (path.inValue() && !_inSubstitutedValue) {
				...
				if (!_doublePositionAllowed) {
					const split = _splitDoublePositionStops(
						fn,
		...
		case T_QUALIFIED_RULE: {
			...
			if (decls === null) {
				...
				if (minify && equalsLowerCase(path.name(), "charset")) return "";
				return `${prelude};`;
			}
		...
		case T_HASH: {
			...
			const lowered = _hexAlphaFallback(text);
