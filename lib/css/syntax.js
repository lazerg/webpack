...
// Chromium's sits up to 1.0e-4 above it, in encoded units, measured over the
// whole range through `color(srgb-linear …)`.
const _TRANSFER_ERROR = 1.5e-4;
...
// magnitude: up to 9.5e-4 of it, over 91k channels of `color()`, `lab()`,
// `lch()`, `oklab()` and `oklch()` read back from Chromium. This is ~1.6x that.
const _CONVERSION_ERROR = 1.5e-3;
...
const _relativeChannel = (text, bound, angle) => {
	...
	const only = [...sum];
	if (only[0][0] !== (angle ? "deg" : "")) return null;
	return only[0][1];
};
