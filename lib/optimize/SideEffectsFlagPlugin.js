...
class SideEffectsFlagPlugin {
	 ...
	 * @param {boolean=} recordStatement keep the statement that made a module have side effects, for `performance.unusedModules` to report
	 */
	constructor(analyseSource = true, recordStatement = false) {
		/** @type {boolean} */
		this._analyseSource = analyseSource;
		/** @type {boolean} */
		this._recordStatement = recordStatement;
	}
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				if (this._analyseSource) {
					const recordStatement = this._recordStatement;
					...
					const applySideEffectsStmtHandler = (parser) => {
						...
						parser.hooks.finish.tap(PLUGIN_NAME, () => {
							if (sideEffectsStatement === undefined) {
								...
								// Kept on the module rather than in the bailout, which the
								// module graph holds and the cache does not restore. Only when
								// something reads it back: every module here would carry one.
								if (recordStatement) {
									/** @type {BuildInfo} */
									(parser.state.module.buildInfo).sideEffectStatement =
										`${type} at ${at}`;
								}
