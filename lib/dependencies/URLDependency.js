...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// can statically follow the asset. `url: "relative"` and entry URLs keep the runtime
		// form. A prefetch/preload hint doesn't force it: the `<link>` is emitted separately
		// at chunk startup (`StartupAssetHintRuntimeModule`), independent of the call site.
		if (
			!isEntryUrl &&
			!dep.relative &&
		...
		if (isEntryUrl) {
			...
			urlExpr = `${RuntimeGlobals.publicPath} + ${getChunkFilename}(${JSON.stringify(chunk.id)})`;
		} else {
			...
			if (wrapperLess && typeof filename === "string") {
				runtimeRequirements.add(RuntimeGlobals.publicPath);
				urlExpr = `${RuntimeGlobals.publicPath} + ${toJsStringLiteral(filename)}`;
			} else {
