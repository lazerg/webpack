...
/**
 * Marks the module as an ES module, as an `import`/`export` statement in the
 * source would. Must run before the body is walked to take effect on it.
 * @param {JavascriptParser} parser the parser
 * @param {boolean} isStrictHarmony strict harmony mode should be enabled
 * @returns {void}
 */
const enableHarmony = (parser, isStrictHarmony) => {
...
class HarmonyDetectionParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.program.tap(PLUGIN_NAME, (ast) => {
			...
			if (isHarmony) {
				enableHarmony(parser, isStrictHarmony);
			}
		});

		parser.hooks.topLevelAwait.tap(PLUGIN_NAME, (node) => {
			const module = parser.state.module;
			enableHarmony(parser, false);
			/** @type {BuildMeta} */
...
HarmonyDetectionParserPlugin.enableHarmony = enableHarmony;

module.exports = HarmonyDetectionParserPlugin;
