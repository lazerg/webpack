...
/**
 * Splits `"<syntax> <name> <alias>"` into its parts, `"|"` and `" "` are both separators.
		...
		for (const value of part.split(" ")) {
			if (!value) {
				throw new WebpackError(
					`Invalid "${item}" value for export, there must be only one separator: " " or "|"`
				);
			}
			result.push(value);
		}
...
const parseExports = (exports) => {
	const items = Array.isArray(exports)
		? exports
...
class InjectedExportsParserPlugin {
	...
	injectHarmonyExports(parser, exports) {
		...
		for (const [index, { syntax, name, alias }] of exports.entries()) {
			...
			innerGraph.addVariableUsage(parser, name, exportName);
			const tagData =
