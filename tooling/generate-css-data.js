...
// The production every grammar that interpolates between colors names.
const INTERPOLATION_METHOD = "color-interpolation-method";

/**
 * The functions that compute with the colors written in them rather than paint
 * one of them, spotted by the `<color-interpolation-method>` their grammars
 * reach: a mix of two, and every gradient, which interpolates between its stops.
 * Inside one of these a color rounded to the byte an engine paints is the wrong
 * color — the engine computes from what was written. `light-dark()` names no
 * interpolation: it picks one of its two and paints it.
 * @returns {string[]} the function names, sorted
 */
const collectColorComputingFunctions = () => {
	/** @type {Map<string, boolean>} */
	const interpolates = new Map();
	// Followed through the productions a function's own grammar is spelled out
	// in, and no further: a call it merely takes a value of — an `<image>` that
	// may be a gradient — interpolates on its own account, not this one's.
	/**
	 * @param {string} production a production name
	 * @returns {boolean} whether its grammar reaches the interpolation method
	 */
	const reaches = (production) => {
		const known = interpolates.get(production);
		if (known !== undefined) return known;
		// A cycle answers `false` until the walk that entered it says otherwise.
		interpolates.set(production, false);
		const entry = syntaxes[production];
		if (entry === undefined || typeof entry.syntax !== "string") return false;
		const children = references(entry.syntax);
		const found =
			children.includes(INTERPOLATION_METHOD) ||
			children.some((child) => !child.endsWith("()") && reaches(child));
		interpolates.set(production, found);
		return found;
	};
	const out = [];
	for (const name of Object.keys(syntaxes)) {
		if (!name.endsWith("()")) continue;
		if (reaches(name)) out.push(name.slice(0, -2));
	}
	if (out.length === 0) {
		throw new Error(`no function reaches <${INTERPOLATION_METHOD}>`);
	}
	return out.sort();
};
...
const collectData = async () => {
	...
	const colorComputingFunctions = collectColorComputingFunctions();
...
// The functions that compute with the colors in them rather than paint one:
// every gradient interpolates between its stops, and a mix interpolates between
// its two. A color rounded to the byte an engine paints is the wrong color
// inside one — what the engine computes from is what was written.
const COLOR_COMPUTING_FUNCTIONS = ${setLiteral(colorComputingFunctions)};
